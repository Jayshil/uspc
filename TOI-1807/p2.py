...
for i in range(len(instruments)):
    tim7, fl7, fle7 = np.loadtxt(os.getcwd() + '/Data/PDCSAP/TOI-1807/' + instruments[i] + '.dat', usecols=(0,1,2), unpack=True)
...
## Occultation time
tc2, tc2_err = tc1 + (0.5 * per), np.sqrt(tc1_err + (0.5 * per_err))

# Priors
## Planetary priors
par_P = ['P_p1', 't0_p1', 'p_p1', 'b_p1', 'q1_' + '_'.join(instruments), 'q2_' + '_'.join(instruments), 'rho', 'ecc_p1', 'omega_p1']
dist_P = ['normal', 'normal', 'truncatednormal', 'truncatednormal', 'uniform', 'uniform', 'normal', 'fixed', 'fixed']
